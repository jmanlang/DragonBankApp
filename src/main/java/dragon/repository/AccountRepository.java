...
public class AccountRepository {
    ...
    public Account findSavingsByOwnerId(Connection connection, UUID ownerId) throws SQLException {
        String sql = "SELECT id, owner, balance FROM SavingAccount WHERE owner = ? LIMIT 1";
    ...
    public boolean updateSavingBalance(Connection connection, UUID accountId, double newBalance) throws SQLException {
        if (newBalance < 0) {
            return false;
        }

        String sql = "UPDATE SavingAccount SET balance = ? WHERE id = ?";
